...
export default function MyTicketsPage() {
  ...
  const myTickets = bookings || [];
  
  const filtered = myTickets.filter(t => {
    const movie = movies.find(m => String(m.id) === String(t.movieId));
    const status = t.status || 'upcoming';
    const matchTab = tab === 'all' || status === tab;
    const matchSearch = String(t.id).toLowerCase().includes(search.toLowerCase()) || (movie?.title ?? '').toLowerCase().includes(search.toLowerCase());
  ...
  return (
      <div className="container mx-auto px-4 py-10 max-w-3xl space-y-6">
        ...
        {filtered.length === 0 ? <div className="text-center py-16 text-muted-foreground">
            ...
            {filtered.map(ticket => {
          const movie = movies.find(m => String(m.id) === String(ticket.movieId));
