...
export default function AdminMoviesPage() {
  ...
  const handleSave = () => {
    const payload = {
      ...formData,
      genres: formData.genres || ['Hành động'], // Default for mock
      ...
      <div className="space-y-6">
        ...
        <div className="grid gap-4 md:grid-cols-3">
          {[{
            ...
            label: 'Đang chiếu',
            value: movies.filter(m => m.status === 'now_showing').length,
            icon: Film
          }, {
            label: 'Sắp chiếu',
            value: movies.filter(m => m.status === 'coming_soon').length,
            icon: Film
                ...
                {filtered.map(movie => <TableRow key={movie.id} className="border-border">
                    ...
                    <TableCell>
                      <div className="flex flex-wrap gap-1 max-w-[200px]">
                        {(movie.genres || []).slice(0, 2).map(g => <Badge key={g} variant="secondary" className="text-xs">
                            {g}
                          </Badge>)}
                      </div>
                    </TableCell>
