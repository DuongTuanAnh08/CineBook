...
export default function AdminPromotionsPage() {
  ...
  const filtered = mockPromos.filter(p => p.code.toLowerCase().includes(search.toLowerCase()) || p.name.toLowerCase().includes(search.toLowerCase()));
  return (
    <div className="space-y-6">
        ...
        <div className="grid gap-4 md:grid-cols-4">
          {[{
          label: 'Tổng mã',
          value: mockPromos.length
        }, {
          label: 'Đang hoạt động',
          value: mockPromos.filter(p => p.status === 'active').length
        }, {
          label: 'Tổng lượt dùng',
          value: mockPromos.reduce((a, p) => a + p.usedCount, 0)
        }, {
          label: 'Sắp diễn ra',
          value: mockPromos.filter(p => p.status === 'upcoming').length
          ...
          <CardHeader>
            <div className="flex items-center gap-3">
              <div className="relative flex-1 max-w-sm">
                ...
                <Input placeholder="Tìm mã hoặc tên khuyến mãi..." className="pl-9" value={search} onChange={e => setSearch(e.target.value)} />
              </div>
                ...
                {filtered.map(promo => <TableRow key={promo.id} className="border-border">
                    ...
                    <TableCell>
                      <span className="text-primary font-semibold">
                        {promo.type === 'percent' ? `-${promo.discount}%` : `-${promo.discount.toLocaleString('vi-VN')}₫`}
                      </span>
                    </TableCell>
                    <TableCell className="text-sm">
                      {promo.minAmount > 0 ? `${promo.minAmount.toLocaleString('vi-VN')}₫` : 'Không giới hạn'}
                    </TableCell>
                    <TableCell>
