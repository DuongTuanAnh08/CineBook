          ...
          <Card className="bg-card border-border">
            ...
            <CardContent>
              <div className="space-y-4">
                {recentBookings.map(booking => <div key={booking.id} className="flex items-center justify-between p-3 rounded-lg bg-secondary/30 hover:bg-secondary/50 transition-colors">
                    ...
                    <div className="text-right">
                      ...
                      <Badge variant={booking.status === 'completed' ? 'default' : booking.status === 'pending' ? 'secondary' : 'destructive'} className={booking.status === 'completed' ? 'bg-green-500/20 text-green-500 hover:bg-green-500/30' : booking.status === 'pending' ? 'bg-yellow-500/20 text-yellow-500 hover:bg-yellow-500/30' : ''}>
                        {booking.status === 'completed' ? 'Hoàn thành' : booking.status === 'pending' ? 'Chờ xử lý' : 'Đã hủy'}
                      </Badge>
                    </div>
                  </div>)}
              </div>
