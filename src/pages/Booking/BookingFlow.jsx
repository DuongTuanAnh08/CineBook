...
function BookingContent() {
  ...
  const activeItems = concessions.filter(i => i.status === 'active');
  ...
  return <div className="container mx-auto px-4 py-6">
      ...
      {/* Seat Selection */}
      <SeatSelection movieId={movie.id} movieTitle={movie.title} moviePoster={movie.poster} cinemaName={cinema.name} roomName={room} showDate={date} showTime={time} pricing={dynamicPricing} onConfirm={handleConfirm} onCancel={() => router(-1)} />
        ...
        <SheetContent side="bottom" className="h-[90vh] flex flex-col bg-card border-t border-border p-0">
          ...
          <div className="flex-1 overflow-y-auto px-6 py-4">
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2">
              {activeItems.filter(i => i.type === activeTab).map(item => {
