...
const PRICING = { standard: 75000, vip: 100000, couple: 180000 }

const ROOM_OPTIONS = cinemas.flatMap((c, ci) =>
  Array.from({ length: [8, 6, 10, 7, 9][ci] ?? 6 }, (_, i) => ({
    id: `${c.id}-${i + 1}`,
...
export default function AdminSeatsPage() {
  const [selectedRoom, setSelectedRoom] = useState<string>(ROOM_OPTIONS[0].id)

  const layout = useMemo(() => getStaticSeatLayout(PRICING), [])

  const totalSeats = layout.rows.reduce((acc, r) => acc + r.seats.length, 0)
  const bookedCount = layout.rows.reduce(
    (acc, r) => acc + r.seats.filter((s) => s.status === 'booked').length,
    0,
  )
  const vipCount = layout.rows.reduce(
    (acc, r) => acc + r.seats.filter((s) => s.type === 'vip').length,
    0,
  )
  const coupleCount = layout.rows.reduce(
    (acc, r) => acc + r.seats.filter((s) => s.type === 'couple').length,
    0,
  )
      ...
      <div className="space-y-6">
        ...
        <div className="grid gap-4 md:grid-cols-4">
          {[
            ...
            { label: 'Ghế thường', value: totalSeats - vipCount - coupleCount },
          ...
          <CardContent>
            ...
            <div className="flex flex-col items-center gap-6 overflow-x-auto pb-2">
                  ...
                  <div key={row.row} className="flex items-center gap-1.5">
                    ...
                    <div className="flex gap-1.5">
                      {row.seats.map((seat, i) => {
                        ...
                        return (
                          <div key={seat.id} className={cn('flex gap-1.5', isAisle && 'ml-3')}>
                            <div
                              className={cn(
                                'w-6 h-6 rounded-t-sm text-[9px] flex items-center justify-center border cursor-pointer transition-colors',
                                seat.status === 'booked'
                                  ? SEAT_COLORS.booked
                                  : seat.type === 'vip'
                                  ? SEAT_COLORS.vip
              ...
              <div className="flex items-center gap-6 text-xs text-muted-foreground mt-2">
                {[
                  { color: 'bg-secondary/60 border border-border', label: 'Thường' },
                  { color: 'bg-yellow-500/20 border border-yellow-500/40', label: 'VIP' },
                  { color: 'bg-pink-500/20 border border-pink-500/40', label: 'Ghế đôi' },
                  { color: 'bg-red-500/30 border border-red-500/50', label: 'Đã đặt' },
                  { color: 'bg-yellow-500/30 border border-yellow-500/50', label: 'Đang giữ' },
                ].map((l) => (
