...
const mockRooms = cinemas.flatMap((cinema, ci) =>
  Array.from({ length: [8, 6, 10, 7, 9][ci] ?? 6 }, (_, i) => ({
    ...
    totalSeats: i % 3 === 0 ? 200 : i % 3 === 1 ? 150 : 120,
    ...
    <AdminLayout>
      <div className="space-y-6">
        ...
        <div className="grid gap-4 md:grid-cols-4">
          {[
            { label: 'Tổng phòng chiếu', value: filtered.length },
            { label: 'Đang hoạt động', value: activeCount },
            { label: 'Bảo trì', value: maintenanceCount },
            {
              label: 'Tổng ghế',
              value: filtered.reduce((a, r) => a + r.totalSeats, 0).toLocaleString(),
            },
          ].map((s) => (
              ...
              <TableBody>
                {filtered.map((room) => (
                  <TableRow key={room.id} className="border-border">
                    ...
                    <TableCell className="text-sm">{room.totalSeats} ghế</TableCell>
                    <TableCell>
                      <Badge
                        className={
                          room.status === 'active'
