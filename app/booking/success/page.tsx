...
import { CheckCircle2, Download, Ticket, Home, Calendar, Clock, MapPin, Armchair, Loader2 } from 'lucide-react'
          ...
          <div className="flex flex-col items-center gap-3">
            <div className="w-32 h-32 bg-secondary rounded-xl grid grid-cols-6 gap-0.5 p-2">
              {Array.from({ length: 36 }, (_, i) => (
                <div
                  key={i}
                  className={`rounded-sm ${[0,1,5,6,7,11,30,31,35,12,18,24,13,19,25].includes(i) ? 'bg-foreground' : 'bg-transparent'}`}
                />
              ))}
            </div>
