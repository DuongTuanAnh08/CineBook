...
import { Film, Calendar, Clock, AlertTriangle, Check } from 'lucide-react'
...
import type { ScheduleShowtime, Room, ScheduleConflict } from '@/types/schedule'
...
interface AddShowtimeModalProps {
  ...
  selectedDate: string
  onAdd: (showtime: Omit<ScheduleShowtime, 'id'>) => void
}
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[500px] bg-card border-border">
        <DialogHeader>
          ...
          <DialogDescription>
            Thêm ca chiếu cho ngày {format(new Date(selectedDate), 'dd/MM/yyyy', { locale: vi })}
          </DialogDescription>
        </DialogHeader>
                ...
                {rooms.map((room) => (
                  <SelectItem key={room.id} value={room.id}>
                    <div className="flex items-center gap-2">
                      ...
                      <Badge 
                        variant="outline" 
                        className={cn("text-[10px] h-5", getRoomTypeColor(room.type))}
                      >
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center h-10 px-3 rounded-md border border-border bg-muted/50 text-muted-foreground">
                {endTime || '--:--'}
                {selectedMovie && (
                  <span className="ml-2 text-xs">({selectedMovie.duration} phút)</span>
                )}
              </div>
            </div>
          </div>
        ...
        <DialogFooter>
          ...
          <Button 
            ...
            disabled={!isValid}
            className={cn(
              hasConflicts && "bg-amber-600 hover:bg-amber-700"
            )}
          >
