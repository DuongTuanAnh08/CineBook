...
export function ShowtimeList({ movieId, showtimes, onSelectShowtime }: ShowtimeListProps) {
        ...
        {Object.keys(groupedByCinema).length > 0 ? (
          <div className="space-y-4">
            {Object.entries(groupedByCinema).map(([cinemaId, cinemaShowtimes]) => {
              ...
              const groupedByRoom: Record<string, Showtime[]> = {}
              ...
              return (
                <Card key={cinemaId} className="overflow-hidden border-border/50 p-0">
                            ...
                            .map((showtime) => {
                              ...
                              return (
                                <Button
                                  ...
                                  className={`group relative flex-col h-auto py-2 px-3 hover:border-primary hover:bg-primary/10 ${
                                    isLowSeats ? 'border-orange-500/50' : ''
                                  }`}
                                  onClick={() => {
                                                                      onSelectShowtime?.(showtime)
                                                                      router.push(
                                                                        `/booking/${movieId}?id=${movieId}&cinema=${showtime.cinemaId}&room=${encodeURIComponent(showtime.roomName)}&date=${selectedDate}&time=${showtime.startTime}`
                                                                      )
                                                                    }}
                                >
                                  ...
                                  <span className="text-xs text-muted-foreground">
                                    {showtime.availableSeats === 0 ? (
                                      <span className="text-destructive">Hết chỗ</span>
                                    ) : (
                                      `${showtime.availableSeats} chỗ`
                                    )}
                                  </span>
