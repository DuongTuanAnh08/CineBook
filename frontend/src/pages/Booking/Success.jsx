...
function SuccessContent() {
  ...
  const concessions = concessionsParam ? (() => {
    try { return JSON.parse(concessionsParam); }
    catch { return []; }
  })() : [];
  ...
  // Gọi API lấy ticket data (bao gồm qrCodeValue) sau khi booking thành công
  // eslint-disable-next-line react-hooks/rules-of-hooks
  useEffect(() => {
    if (bookingId) {
      setQrLoading(true);
      bookingApi.getBookingById(bookingId)
        .then(res => { if (res.success) setTicketData(res.data); })
        .catch(console.error)
    ...
    <Card className="bg-card border-border overflow-hidden">
      ...
      <CardContent className="pt-6 pb-5 space-y-4">
        <div className="grid grid-cols-2 gap-4 text-sm">
          {[
            { icon: Calendar, label: 'Ngày chiếu', value: displayDate },
            { icon: Clock,    label: 'Suất chiếu', value: time },
            { icon: MapPin,   label: 'Phòng',      value: room },
            { icon: Armchair, label: 'Ghế',        value: seats.join(', ') }
        ...
        {/* QR Code động — BookingQRCard nếu có ticket data, fallback QR từ bookingId */}
        <div className="flex flex-col items-center gap-3">
          {qrLoading ? (
            ...
            <BookingQRCard tickets={ticketData.tickets} size={150} />
          ) : (
